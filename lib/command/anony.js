...
Contoh : ${prefix}menfes 6283862323152 | I Have a Crush On You`);
...
Contoh : ${prefix}confes 6283862323152 | Aku Sebenarnya Sudah Suka Kamu Dari Lama Tapi Kamu Ga peka:v | Inisial M Temen Sekelasmu`);
    jid = jid + "@s.whatsapp.net"
    //         if (!(await client.onWhatsApp(jid))[0]) return client.reply(msg, "_Nomor Whatsapp Yang Anda Masukkan Tidak Aktif_ \
    // *_Mohon periksa nomor yang anda kirimkan_*")
    let sensor = ["kontol", "ngentot", "memek", "anjing"].forEach(tr => {
    ...
    await client.sendAdReply(jid, { text: `*Haii Orang Yang Menyukaimu Mengirim Pesan Nihh*\nPesan:` + teks + `\n\nClue:` + clue || "-" + `\n\nTag/Balas?Reply Pesan ini Untuk Membalas Pesan Tersebut\nConfest MechaBot`, mentions: [msg.sender.jid] }, { title: "Confest MechaBot", description: config.botname, matchedText: global.linkgcme, canonicalUrl: global.linkgcme, thumbnail: config.mecha_image, previewType: 2, renderLargerThumbnail: true, mediaType: 1 })
    await client.sendButton(msg, {
      text: "Sukses Mengirim Pesan", footer: `Sukses Mengirim Pesan
 -> Kontak : wa.me/${jid.split('@')[0]}
 -> Pesan : ${teks} 
...
${config.botname}`
  ...
  query: "Penggunaan #menfess nomor|pesan untuknya\n\nContoh : #menfess 6283862323152 | I Have a Crush On You\n\n\nPenggunaan #confes nomor|pesan|clue untuknya\n\nContoh : #confes 6283862323152 | Aku Sebenarnya Sudah Suka Kamu Dari Lama Tapi Kamu Ga peka:v | Inisial M Teman Sekelas Mu",
  private: true
})
...
// cmd.on(["", 'balas', 'bls'], [], async (msg, {
//   query, command, client, prefix, quotedMsg
// }) => {
//   if (msg.quotedMsg && msg.quotedMsg.mentionedJid[0]) {
//     let teks = msg.string
//     let sensor = ["kontol",
