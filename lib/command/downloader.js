...
*${config.botname}*`
  ...
  Data Berhasil Didapatkan:\n`
            ...
            let filename = "Convert By: MechaBot "
            client.sendButton(msg, { video: data[1].url, content: captions, footer: config.botname }, [{ reply: "Document", value: `.ytmp4doc ${sul}` }, { reply: "Audio", value: `.ytmp3 ${sul}` }])
        } catch (err) {
            client.reply(msg, `Terjadi Kesalahan Saat Mengambil Data Tersebut\n\n` + err)
        }
    }
...
*${config.botname}*`
                ...
                let sul = play.result[0].url
                let data = await yutube("mp3", sul)
                let filename = "Convert By: MechaBot " + play.result[0].title
                return client.sendAdReply(msg, { document: data[1].url, fileName: filename }, { title: play.result[0].title, body: play.result[0].type , thumbnail: play.result[0].thumbnail, mediaType: 1, mediaUrl: sul, sourceUrl: sul })
            }
            if (command == "ytmp3doc") {
                if (urls[0].includes("/shorts/")) {
                    let data = await yutube("mp3", sul)
