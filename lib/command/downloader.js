...
${head}${line.repeat(3)}${list}`
                ...
                client.sendButton(msg, { video: result.data[1].url, fileName: filename, content: captions, footer: config.botname }, [{ reply: "Document", value: `.ytmp4doc ${urls[0]}` }, { reply: "Audio", value: `.ytmp3 ${urls[0]}` }])
            }
  ...
  Data Berhasil Didapatkan:\n`
            ...
            let filename = result.title + " Convert By: MechaBot"
            client.sendButton(msg, { video: result.data[1].url, content: captions, footer: config.botname }, [{ reply: "Document", value: `.ytmp4doc ${sul}` }, { reply: "Audio", value: `.ytmp3 ${sul}` }])
        } catch (err) {
            client.reply(msg, `Terjadi Kesalahan Saat Mengambil Data Tersebut\n\n` + err)
        }
    }
  ...
  Data Berhasil Didapatkan:\n`
    ...
    if (command == "play" || command == "lagu" || command == "song") {
        try {
            ...
            //   client.sendButton(msg, { image: play.result[0].thumbnail, content: captions + "\n\n_Mohon Tunggu Sebentar_", footer: config.botname }, [{ reply: "Video", value: `ytmp4 ${sul}` }, { reply: "Document", value: `ytmp3doc ${sul}` }])
            let filename = result.title + " Convert By: MechaBot"
            await client.sendAdReply(msg, { audio: result.data[1].url, fileName: filename }, { title: result.title, body: result.data[1].bitrate + ` || ` + result.data[0].size, thumbnail: result.thumb, mediaType: 2, mediaUrl: urls[0], sourceUrl: urls[0] })
        } catch (err) {
