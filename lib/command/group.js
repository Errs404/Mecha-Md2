...
cmd.on(['mute', 'unmute'], ['group'], async (msg, {
  ...
  if (command == "mute") {
    database[msg.from].mute.push(msg.from)
    client.reply(msg, `sucess mute in gc ${msg.groupData.subject}`)
  }
  if (command == "unmute") {
    database[msg.from].mute.splice(msg.from)
...
cmd.on(['autostick', 'autosticker'], ['group'], async (msg, {
  ...
  if (query.endsWith('hidup') || query.endsWith('on')) {
    ...
    database[msg.from].autosticker.push(msg.groupData.id)
  ...
  if (query.endsWith('mati') || query.endsWith('off')) {
    database[msg.from].autosticker.splice(msg.groupData.id)
    // fs.writeFileSync('./src/json/antilink.json', JSON.stringify(database[msg.from].autosticker))
...
cmd.on(['welcome', 'leave', 'setwelcome', 'setleave'], ['group'], async (msg, {
  ...
  if (command == "welcome") {
    ...
    if (query.endsWith('mati') || query.endsWith('off')) {
      if (database[msg.from].welcome.includes(msg.groupData.id)) return client.reply(msg, { text: `sepertinya welcome sudah dimatikan sebelumnya` })
      database[msg.from].welcome.push(msg.groupData.id)
      fs.writeFileSync('./src/json/welcome.json', JSON.stringify(database[msg.from].welcome))
      return client.reply(msg, `Welcome Di Group *${msg.groupData.subject}* Telah di Matikan Oleh Admin Group`)
    }
    if (query.endsWith('hidup') || query.endsWith('on')) {
      database[msg.from].welcome.splice(msg.groupData.id)
      fs.writeFileSync('./src/json/welcome.json', JSON.stringify(database[msg.from].welcome))
  ...
  if (command == "leave") {
    ...
    if (query.endsWith('mati') || query.endsWith('off')) {
      if (database[msg.from].leave.includes(msg.groupData.id)) return client.reply(msg, { text: `sepertinya leave sudah dimatikan sebelumnya` })
      database[msg.from].leave.push(msg.groupData.id)
    ...
    if (query.endsWith('hidup') || query.endsWith('on')) {
      database[msg.from].leave.splice(msg.groupData.id)
      fs.writeFileSync('./src/json/leave.json', JSON.stringify(database[msg.from].leave))
