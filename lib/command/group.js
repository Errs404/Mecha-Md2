...
cmd.on(['welcome', 'leave', 'setwelcome', 'setleave'], ['group'], async (msg, {
  ...
  if (command == "setwelcome") {
    if (!query) {
      ...
      if (!database[msg.from].setwelcome == {}) {
        ...
        return database[msg.from].setwelcome = JSON.stringify(objekwelcome, null, 2);
      } else {
        const objekwelcome = {
          status: true,
          group: msg.from,
  ...
  if (command == "setleave") {
    if (!query) {
      ...
      if (!database[msg.from].setleave == {}) {
        ...
        return database[msg.from].setleave = JSON.stringify(objekwelcome, null, 2);
      } else {
        const objekwelcome = {
          status: true,
          group: msg.from,
