...
const { configureSuccessfulPairing, jidEncode } = require("@whiskeysockets/baileys")
const fs = require("fs");
...
cmd.on(['linkgc', 'linkgroup', 'linkgrup'], ['group'], async (msg, {
  ...
  client.sendButton(msg, { text: `Link Group ${msg.groupData.subject}`, footer: config.botname }, [{ url: "Click To Copy", value: "https://www.whatsapp.com/otp/copy/" + link2, index: 1 }, { url: "Group Mecha", value: global.linkgcme, index: 1 }])
}, {
})
...
cmd.on(['autostick', 'autosticker'], ['group'], async (msg, {
  ...
  if (query.endsWith('hidup') || query.endsWith('on')) {
    if (database[msg.from].autosticker) return client.reply(msg, { text: `sepertinya auto sticker sudah aktif sebelumnya` })
...
cmd.on(['welcome', 'leave', 'setwelcome', 'setleave'], ['group'], async (msg, {
  ...
  if (command == "setwelcome") {
    if (!query) {
      ...
      if (Object.keys(database[msg.from].setwelcome).length == 4) {
        ...
        client.reply(msg, `sukses Welcome telah di setel ulang menjadi ${database[msg.from].setwelcome.data.text}`)
      } else {
        const objekwelcome = {
          status: true,
          group: msg.from,
        ...
        database[msg.from].setwelcome = objekwelcome
        client.reply(msg, `sukses Welcome telah di setel menjadi ${database[msg.from].setwelcome.data.text}`)
      }
    }
  ...
  if (command == "setleave") {
    if (!query) {
      ...
      if (!Object.keys(database[msg.from].setleave).length != 0) {
        ...
        client.reply(msg, `sukses leave telah di setel ulang menjadi ${database[msg.from].setwelcome.data.text}`)
      } else {
        const objekwelcome = {
          status: true,
          group: msg.from,
        ...
        database[msg.from].setleave = objekwelcome
        client.reply(msg, `sukses leave telah di setel menjadi ${database[msg.from].setwelcome.data.text}`)
      }
    }
    if (query == "%default") {
