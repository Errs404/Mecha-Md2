...
cmd.on(['faceanime', 'animeface', 'jadianime', 'fp', 'toanime', 'to2d', 'togepeng'], ['maker'], async (msg, { client, query }) => {
  try {
    let down = await msg.isMedia ? msg.downloadMsg : msg.quotedMsg.downloadMsg
    let { toAnime } = require("../scrapper/toanime")
    down = await down()
    let convert = await toAnime(down)
    return await client.reply(msg, { image: JSON.parse(convert.extra).img_urls[0], caption: `SUSCESS\n Crop sendiri ya om!\n by: MechaBot ` })
  } catch (e) {
    client.reply(msg, `Wajah tidak terdeteksi cobalah dengan menggunakan foto lain!! \n\n jangan Gambar Anime.. Uda anime dibuat anime ente kadang-kadang \n\n ${e}`)
  }
}, {
  _media: true,
  wait: "*WAIT! | Mohon Tunggu Sebentar..*",
