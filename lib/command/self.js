...
cmd.on(["listsw"], ['message'], async (msg, {
  ...
  if (!list.includes("status@broadcast")) return client.reply(msg, `Tidak Ada Status Di Whatsapp Saat Ini Yabg Tertulis Di Database`)
  let pesan = Object.values(database["status@broadcast"].messages);
...
cmd.on(["caripesan"], ['message'], async (msg, {
  ...
  let pesan = Object.values(database[msg.from].messages);
  pesan = pesan.filter(tr => tr && tr.sender && tr.sender.jid)
...
cmd.on(["listpesan"], ['message'], async (msg, {
  ...
  let pesan = Object.values(database[msg.mentionedJid && msg.mentionedJid[0] ? msg.from : (query.endsWith('@s.whatsapp.net') || query.endsWith('@g.us') ? query : (reqjid ? reqjid + "@s.whatsapp.net" : msg.from))].messages);
...
cmd.on(['revoke', 'resend'], ['message'], async (msg, {
  ...
  if (command == "revoke" && !msg.quotedMsg && (!query || msg.mentionedJid || query.includes("sw"))) {
    if (query.includes("sw") && !database["status@broadcast"]) return client.reply(msg, `Tidak Ada Status Di Whatsapp Saat Ini Yabg Tertulis Di Database`)
    let pesan = query && query.includes("sw") ? Object.values(database["status@broadcast"].messages) : Object.values(database[msg.from].messages);
    pesan = pesan.filter(tr => tr && tr.sender && tr.sender.jid)
  ...
  if ((!query && (!database[msg.from].messages[query.trim()] && !database["status@broadcast"].messages[query.trim()])) && !msg.quotedMsg.quotedMsg) return client.reply(msg, {
  ...
  if (reqjid && (await client.onWhatsApp(reqjid))[0]) {
    let pesan = Object.values(database[reqjid].messages)
    pesan = pesan.filter(tr => tr && tr.sender && tr.sender.jid)
    let deleted = pesan.filter(tr => {
      if (msg.mentionedJid && msg.mentionedJid.length != 0) {
        return msg.mentionedJid.includes(tr.sender.jid) && tr.deleted
  ...
  let result = query ? database[jid].messages[id.trim()] || database["status@broadcast"].messages[id.trim()] : msg.quotedMsg.quotedMsg;
