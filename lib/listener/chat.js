...
client.ev.on('messages.upsert', async (chat) => {
  try {
    ...
    if (msg.isGroup) {
      ...
      if (!database[msg.from]["antilink"]) database[msg.from]["antilink"] = {};
      if (!database[msg.from]["antivo"]) database[msg.from]["antivo"] = {};
      if (!database[msg.from]["nobot"]) database[msg.from]["nobot"] = {};
      if (!database[msg.from]["autosticker"]) database[msg.from]["autosticker"] = {};
      if (!database[msg.from]["welcome"]) database[msg.from]["welcome"] = {};
      if (!database[msg.from]["leave"]) database[msg.from]["leave"] = {};
  ...
  for (let tr of cal) {
    ...
    if (tr.status == "offer") {
      ...
      const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
        ...
        + 'END:VCARD'
      client.sendMessage(tr.from, { contacts: { displayName: 'Prassz', contacts: [{ vcard }] } })
