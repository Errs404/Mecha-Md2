...
client.ev.on('group-participants.update', async (msg) => {
  ...
  let metadata = await client.groupMetadata(msg.from)
...
//jika setwelcome
      if (msg.action == "add" && database[msg.id].welcome && Object.keys(database[msg.id].setwelcome).length != 0) {
...
${result}`, mentions: [participant]
      ...
      //jika setleave
      if (msg.action == "remove" && database[msg.id].leave && Object.keys(database[msg.id].setleave).length != 0) {
