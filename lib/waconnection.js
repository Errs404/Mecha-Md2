...
module.exports = class WAConnection {
  ...
  async getBuffer(path, save) {
    ...
    })).buffer() : fs.existsSync(path) ? fs.readFileSync(path) : path._readableState ? await this.toBuffer(path) : typeof path === 'string' ? path : Buffer.alloc(0);
  ...
  async sendMessageFromContent(mess, obj = {}) {
    ...
    let prepare = await require("@whiskeysockets/baileys").generateWAMessageFromContent(typeof mess == 'object' ? mess.key.remoteJid : mess, obj, obj)
  ...
  // async prepareSticker(path, options) {
  ...
  async sendAdReply1(msg, message = {}, additional = {}, opt = {}) {
    ...
    additional.thumbnail = (await this.getBuffer(additional.thumbnail)).buffer
  ...
  async sendpool2(jid, name, options, opt = {}) {
    ...
    let generate = await require('@whiskeysockets/baileys').generateWAMessageFromContent(
  ...
  async editMessage(mess, message, opt = {}) {
    let prep = this.prepareMessage("0@s.whatsapp.net", message, opt)
    return await this.sendMessageFromContent(mess.key ? mess : mess.remoteJid, {
      protocolMessage: {
        key: mess.key ? mess.key : mess,
        type: "MESSAGE_EDIT",
        editedMessage: prep.message
      }
    }, { ...opt }
    )
  }
